...
function DirectoryCompanyCard({
  ...
  // If we have an id, make the whole card clickable; otherwise render a plain card.
  const CardComponent = resolvedId ? RouterLink : "div";
  const cardProps = resolvedId ? { to: `/companies/${resolvedId}` } : {};
  ...
  return (
    <Card
      ...
      sx={{
        ...
        "&:hover": resolvedId ? { transform: "translateY(-2px)" } : undefined,
      }}
    >
      <MDBox p={2} display="flex" flexDirection="column" height="100%">
        ...
        >
          <MDTypography
            variant="button"
            color="secondary"
            sx={{ maxWidth: 260, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}
          >
